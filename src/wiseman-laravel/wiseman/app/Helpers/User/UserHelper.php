...
{
    const USER_PHOTO_DIRECTORY = 'user-photo';
    private $userModel;
    ...
    */
    ...
    {
        if (!empty($payload['photo_profile'])) {
            $fileName = $this->generateFileName($payload['photo_profile'], 'USER_' . date('Ymdhis'));
            $photo_profile = $payload['photo_profile']->storeAs(self::USER_PHOTO_DIRECTORY, $fileName,
            'public');
            $payload['photo_profile'] = $photo_profile;
        } else {
            unset($payload['photo_profile']);
        }
    ...
    *
    * @param string $id id dari tabel m_user
    *
    * @return bool
    */
